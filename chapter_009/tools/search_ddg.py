...
def search_ddg(query, max_result_num=5):
    ...
    このツールから得られる情報は非常に簡略化されており、時には古い情報の場合もあります。

    必要な情報が見つからない場合は、必ず `fetch_page` ツールを使用して各ページの内容を確認してください。
    文脈に応じて最も最適な言語を使用してください( ユーザーの言語と同じである必要はありません)
    ...
    - url
    """
    res = DDGS().text(query, region="wt-wt", safesearch="Off", timelimit="lite")
    return [
        {
            "title": r.get('title', ""),
            "snippet": r.get('body', ""),
